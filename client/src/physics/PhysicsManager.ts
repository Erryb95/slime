...
export class PhysicsManager {
  ...
  createRigidBody(
    ...
    if (!this.world || !RAPIER) return null
  ...
  createCharacterController(position: Vector3, radius: number = 0.5, height: number = 1.8) {
    if (!this.world || !RAPIER) return null
  ...
  moveCharacter(
    ...
    if (!this.world || !RAPIER || !characterController) return
  ...
  raycast(origin: Vector3, direction: Vector3, maxDistance: number = 1000) {
    if (!this.world || !RAPIER) return null
