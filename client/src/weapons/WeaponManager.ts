...
export class WeaponManager {
    ...
    private _currentSkin: string = 'default'; // Stored for future UI skin selection
        ...
        try {
            ...
            if (weaponMeshes && weaponMeshes.length > 0) {
                ...
                this.currentWeapon = {
                    ...
                    weaponConfig: {
                        name: weaponName,
                        displayName: weaponConfig.model.replace('.babylon', '').replace('models/', ''), // Use filename as display name
                ...
                // Update weapon position if attached to a bone
                this.updateWeaponPosition();
                
                console.log(`Switched to weapon: ${weaponName} with skin: ${skinName}`);
                return true;
    ...
    fire(origin: Vector3, direction: Vector3, onHit?: (hit: PickingInfo) => void): boolean {
        ...
        if (weapon.weaponConfig.muzzleFlash && this.visualEffectsManager) {
            // TODO: Implement muzzle flash effects using origin and direction
            // const muzzleOffset = new Vector3(0, 0, 1); // Forward from weapon
            // const muzzlePosition = origin.add(muzzleOffset);
            // this.visualEffectsManager.playMuzzleFlash(muzzlePosition, direction);
        }
    ...
    async reload(): Promise<boolean> {
        ...
        // deltaTime can be used for smooth weapon animations and effects
