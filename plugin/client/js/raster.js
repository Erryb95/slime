...
(function (root, factory) {
  ...
  function prepareItems(items, opts, fsMod) {
    ...
    var out = (items || []).map(function (it) {
      ...
      // merge with module 1: one ring group (even-odd = the traced holes stay holes) and a box for cluster.planPieces,
      // so the image becomes an ordinary cluster member (merged with its cut line / overlapping artwork)
      var copy = Object.assign({}, it, { rings: res.rings, rg: res.rings.map(function () { return 0; }) });
      var bx = (it.box || it.bounds || [Infinity, -Infinity, -Infinity, Infinity]).slice();   // [l,t,r,b], + the offset contour
      res.rings.forEach(function (r) { r.forEach(function (q) {
        if (q[0] < bx[0]) bx[0] = q[0]; if (q[0] > bx[2]) bx[2] = q[0]; if (q[1] > bx[1]) bx[1] = q[1]; if (q[1] < bx[3]) bx[3] = q[1];
      }); });
      if (bx[2] >= bx[0]) copy.box = bx;
