...
(function (root, factory) {
  ...
  function freeRegions(item, piece, o) {
    var g = G(), Cl = L();
    var rings = (item.rings || []).map(function (r) { return g.cleanRing(r); })
      .filter(function (r) { return r.length >= 3 && g.area(r) >= 0.5; });
    if (!rings.length || !piece.polygon) return [];
    var filled = exec(Cl.ClipType.ctUnion, rings.map(function (r) { return toPath(r); }), null, Cl.PolyFillType.pftEvenOdd);
  ...
  /**
   * items : [{i, rings}] (document pt) as exported; pieces: geometry.buildPieces(items) (same ids = item.i)
   * opts  : see DEFAULTS
  ...
  function planHoles(items, pieces, opts) {
    ...
    var ok = (pieces || []).filter(function (p) { return p && !p.error && p.polygon && itemById[p.id]; });
    ...
    ok.forEach(function (p) {
      freeRegions(itemById[p.id], p, o).forEach(function (r) { r.owner = p.id; regions.push(r); });
    });
    ...
    for (var ri = 0; ri < regions.length; ri++) {
      ...
      for (var ci = 0; ci < cands.length; ci++) {
        ...
        res.children.push({ id: cd.id, parent: R.owner, a: best.a, tx: dx - ra[0], ty: dy - ra[1] });
  ...
  return { DEFAULTS: DEFAULTS, planHoles: planHoles, nestPieces: nestPieces, excludeChildren: excludeChildren,
           expandMoves: expandMoves, movesFor: movesFor,
